...
class WaveNetModel(object):
    ...
    def _generator_dilation_layer(self, input_batch, state_batch, layer_index,
        ...
        if local_condition is not None:
            ...
            output_filter = output_filter + \
                            tf.matmul(local_condition,weights_lcond_filter[0,:,:])
            output_gate = output_gate + \
                          tf.matmul(local_condition, weights_lcond_gate[0,:,:])
    ...
    def _create_generator(self, input_batch, global_condition = None,local_condition=None):
        ...
        q = tf.FIFOQueue(
            1,
            dtypes=tf.float32,
            shapes=(self.batch_size, self.residual_channels),name='queue1')
        init = q.enqueue_many(
